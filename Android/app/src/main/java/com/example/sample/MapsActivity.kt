...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private var distanceCrash: Float = 0.0f
...
//                Log.e("response:", "StartAddress: $startAddress ,EndAddress: $endAddress")
                ...
                GlobalScope.launch(Dispatchers.Main) {
                    ...
                    if (diff > 5) {
                        ...
                        val URL = "https://api.thingspeak.com/update?api_key=VU3IXS40QEL7RENH&field3=$speed&field5=2"
        ...
        binding3.slDistance.addOnSliderTouchListener(object : Slider.OnSliderTouchListener{
            override fun onStartTrackingTouch(slider: Slider) {

            }
            override fun onStopTrackingTouch(slider: Slider) {
                distanceCrash = slider.value
                val URL = "https://api.thingspeak.com/update?api_key=VU3IXS40QEL7RENH&field4=$distanceCrash"
                val client = OkHttpClient()
                val request: Request = Request.Builder().url(URL).build()
                client.newCall(request).enqueue(object : Callback {
                    override fun onFailure(call: Call, e: IOException) {
            ...
            if (response.isSuccessful) {
                ...
                if (rows.length() > 0) {
                    ...
                    if (elements.length() > 0) {
                        ...
                        speed = ((distanceValue/durationInTrafficValue)*0.0372822).toFloat()
